...
def add_fake_labels(fake_label,args):
    ...
    fake_label_path = './data_press/fake_label_data/{}_addfake_train.txt'.format(t)
    test_label_path = './data_press/{}_train.txt'.format(t)
...
def add_fake_test_labels(fake_label,args):
    ...
    fake_label_path = './data_press/fake_label_data/test/{}_addfake_test.txt'.format(t)
    test_label_path = './data_press/{}_test.txt'.format(t)
    ff = open(fake_label_path,'w')  #打开一个文件，可写模式
    with open(test_label_path,'r') as f:  #打开一个文件只读模式
         line = f.readlines()
    i = 0
...
def recompute_memory(epoch, net, lemniscate, trainloader):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, domainIDs, indexes,path) in enumerate(temploader):
