...
def train_model(args, model, dset_loaders, epoch):
    if args.dataset == 'office_home':
        ...
        if tt == 'a':
            ...
            raise NotImplementedError
    with open('/root/MRL-main/data_press/{}/{}_train.txt'.format(args.dataset,s), 'r') as file:
        all_lines = file.readlines()
        
    with open('/root/MRL-main/data_press/{}/{}_train.txt'.format(args.dataset,t), 'r') as file:
    ...
    #criterion = GCELoss(q=0.01).cuda(),q越小越靠近CEW
    ...
    for batch_idx, (inputs_src1, _, domainIDs_src, indexes_src,path_src) in enumerate(source_loader):
        ...
        for i in range(len(args.cluster_num_list)):
            if args.in_domain:
                ...
                #logits_mem_src2 = model.top_layer_tgt[i](model.lemniscate_s.memory[indexes_src])
                #loss_iss1 += criterion(logits_src1[i], (logits_mem_src1 / tau).softmax(1).detach())
                #print((logits_mem_src1 / tau).shape)

                #print(logits_src1[i].shape)
                #print((logits_mem_src1 / tau).softmax(1).detach().shape)
                ...
                if os.path.exists('/root/MRL-main/data_press/{}/{}/{}_cluster_train.txt'.format(args.dataset,args.dset,s)):
                    with open('/root/MRL-main/data_press/{}/{}/{}_cluster_train.txt'.format(args.dataset,args.dset,s), 'r') as file1:
                        line = file1.readlines()
                else:
                    line = all_lines
                file_path = '/root/MRL-main/data_press/{}/{}/{}_cluster_train.txt'.format(args.dataset,args.dset,s)
                if i == 0:
                    for z in range(a):
                ...
                #logits_mem_tar2 = model.top_layer_tgt[i](model.lemniscate_t.memory[indexes_tar])
                ...
                if os.path.exists('/root/MRL-main/data_press/{}/{}/{}_cluster_train.txt'.format(args.dataset,args.dset,t)):
                    with open('/root/MRL-main/data_press/{}/{}/{}_cluster_train.txt'.format(args.dataset,args.dset,t),'r') as file2:
                        line1 = file2.readlines()
                else:
                    line1 = all_lines1
                file_path1 = '/root/MRL-main/data_press/{}/{}/{}_cluster_train.txt'.format(args.dataset,args.dset,t)
                if i == 0:
                    for z1 in range(b):
