...
class data(data.Dataset):
    def __init__(self, dataset, noisy_ratio, mode, noise_mode='sym', root_dir='mydata/', noise_file=None, pred=False, probability=[], log=''):
        self.r = noisy_ratio # noise ratio
        self.mode = mode
        doc2vec = True

        if 'wiki' in dataset.lower():
            path = os.path.join(root_dir, 'p2r_resnet50_MRL.h5')  # wiki_deep_doc2vec_data
            valid_len = 231
        ...
        if doc2vec:
            ...
            if self.mode == 'test' or self.mode == 'valid':
                test_imgs_deep = h['source_test'][()].astype('float32')
                test_imgs_labels = h['source_test_lab'][()]
                test_imgs_labels -= np.min(test_imgs_labels)
                try:
                    test_texts_idx = h['target_test'][()].astype('float32')
                except Exception as e:
                    test_texts_idx = h['target_test'][()].astype('float32')
                test_texts_labels = h['target_test_lab'][()]
                test_texts_labels -= np.min(test_texts_labels)
                test_data = [test_texts_idx, test_imgs_deep]
                test_labels = [test_texts_labels, test_imgs_labels]

                valid_flag = True
                try:
                    valid_texts_idx = h['target_val'][()].astype('float32')
                except Exception as e:
                    try:
                        valid_texts_idx = h['target_val'][()].astype('float32')
                    except Exception as e:
                ...
                if valid_flag:
                    valid_imgs_deep = h['source_val'][()].astype('float32')
                    valid_imgs_labels = h['source_val_lab'][()]
                    valid_texts_labels = h['target_val_lab'][()]
                    valid_texts_labels -= np.min(valid_texts_labels)
                    valid_data = [valid_texts_idx,valid_imgs_deep]
                    valid_labels = [valid_texts_labels,valid_imgs_labels ]

                train_data = valid_data if self.mode == 'valid' else test_data
                train_label = valid_labels if self.mode == 'valid' else test_labels
            elif self.mode == 'train':
                tr_img = h['source_train'][()].astype('float32')
                #print(tr_img.shape(1))
                tr_img_lab = h['source_train_lab'][()]
                tr_img_lab -= np.min(tr_img_lab)
                try:
                    tr_txt = h['target_train'][()].astype('float32')
                except Exception as e:
                    tr_txt = h['target_train'][()].astype('float32')
                tr_txt_lab = h['target_train_lab'][()]
                tr_txt_lab -= np.min(tr_txt_lab)
                train_data = [tr_txt, tr_img]
                train_label = [tr_txt_lab, tr_img_lab]
        ...
        if self.mode == 'train':
            if os.path.exists(noise_file):
                ...
                inx = np.arange(class_num)#产生一个数组
                np.random.shuffle(inx)#将序列中所有元素随机排列
