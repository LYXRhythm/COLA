...
def main():
    ...
    for v in range(n_view):
        if v == args.views.index('Img'):  # Images
            ...
            multi_models.append(models.__dict__['TextNet'](input_dim=512, output_dim=args.output_dim).cuda())
    ...
    if args.loss == 'CE':
        ...
        criterion = utils.GECELoss(q=0.5).cuda()
    elif args.loss == 'FL':
    ...
    def train(epoch):
        ...
        summary_writer.add_scalars('Accuracy/train',
                                   {'view_%d_acc': correct_list[v] / total_list[v] for v in range(n_view)}, epoch)
    ...
    def eval(data_loader, epoch, mode='test'):
        ...
        with torch.no_grad():
            if 1 == 1:
                ...
                for batch_idx, (batches, targets, index) in enumerate(data_loader):
                    batches, targets = [batches[v].cuda() for v in range(n_view)], [targets[v].cuda() for v in
                                                                                    range(n_view)]
                ...
                for v in range(n_view):
                    ...
                    for ct in range(count):
                        ...
                        targets = torch.Tensor(data_loader.dataset.noise_label[v][ct * data_loader.batch_size: (
                                                                                                                           ct + 1) * data_loader.batch_size]).long().cuda()
                        batch = []
        ...
        summary_writer.add_scalars('Accuracy/' + mode,
                                   {('view_%d_acc' % v): correct_list[v] / total_list[v] for v in range(n_view)}, epoch)
        return fea, lab
    ...
    def test(epoch):
        ...
        MAPs = np.zeros([n_view, n_view])
        train_dict = {}
        for i in range(n_view):  # ij代表不同模态，文本或者图像。此处0是Img，是TEXT
            for j in range(n_view):
                MAPs[i, j] = fx_calc_map_label(fea[j], lab[j], fea[i], lab[i], k=0, metric='cosine')[0]
                # print(lab[j])
                train_dict['%s2%s' % (args.views[i], args.views[j])] = MAPs[i, j]

        train_avg = MAPs.sum() / n_view / (n_view - 1.)  # 这里应该除4
        ...
        if val_avg > best_acc:
            ...
            state['C'] = C
            torch.save(state, os.path.join(args.ckpt_dir,
                                           '%s_%s_%d_best_checkpoint.t7' % ('MRL', args.data_name, args.output_dim)))
        return val_dict
    ...
    param_grid = {'beta': [0.1, 0.5, 0.9]}  # 示例参数范围，你可以根据需求修改

    # 创建 GridSearchCV 对象
    grid_search = GridSearchCV(estimator=multi_models, param_grid=param_grid, scoring='accuracy', cv=5)
    X_train = np.array([data[0] for data in train_loader])  # 获取所有的特征
    y_train = np.array([data[1] for data in train_loader])  # 获取所有的标签
    # 进行参数搜索
    grid_search.fit(X_train, y_train)  # X_train, y_train 是你的训练数据
