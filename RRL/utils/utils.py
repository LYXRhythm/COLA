...
def creat(resize_size=256, crop_size=224):
    return transforms.Compose([
...
def get_features(loader, netF):
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            if start_test:
                all_output = outputs.cpu().detach().numpy()#返回值是numpy类型
                all_label = gt_labels.detach().numpy()
                start_test = False
            else:
                try:
                    all_output = np.concatenate((all_output, outputs.cpu().detach().numpy()), 0)
                    all_label = np.concatenate((all_label, gt_labels.detach().numpy()), 0)#数组拼接
                except:
                    print()
        #print(all_output)
...
def eval_AP_inner(inst_id, scores, gt_labels, top=None):# mapi = eval_AP_inner(gt_labels_query[fi], scores[fi], gt_labels_gallery)
    pos_flag = gt_labels == inst_id
    
    tot = scores.shape[0]#tot是原域测试集图像数
    
    tot_pos = np.sum(pos_flag)#原域与此搜索图像中类别相同的个数
 
    sort_idx = np.argsort(-scores)#按降序排列返回的是索引值
   ...
   # print(rec)
    ap = VOCap(rec, prec)
    return gt_labels[sort_idx[0]],ap
...
def cal_map_sda(features_query, gt_labels_query, features_gallary, gt_labels_gallery):
    ...
    #print(scores)scores的行数为原域的特征数，列数为目标域的特征数
    fake_labelling= np.zeros((features_query.shape[0],), dtype = int)

    
    for fi in range(features_query.shape[0]):#.shape[0]为矩阵的行数,
        fake_labelling[fi],mapi = eval_AP_inner(gt_labels_query[fi], scores[fi], gt_labels_gallery)
        try:
            mAP_ls[gt_labels_query[fi]].append(mapi)
           # print(gt_labels_query[fi])
        except:
            print()
    #print(mAP_ls)
...
def add_fake_labels(fake_label,args):
    ss = args.dset.split('2')[0]
    tt = args.dset.split('2')[1]
    if ss == 'a':
         s = 'Art'
    ...
    fake_label_path = 'C:/Users/10189/Desktop/CoDA-main1/data_press/fake_label_data/{}_addfake_train.txt'.format(t)
    test_label_path = 'C:/Users/10189/Desktop/CoDA-main1/data_press/{}_train.txt'.format(t)
    ff = open(fake_label_path,'w')  #打开一个文件，可写模式
    with open(test_label_path,'r') as f:  #打开一个文件只读模式
         line = f.readlines()
    i = 0
    for line_list in line:
        line_new =line_list.replace('\n','')  #将换行符替换为空('')
        b = str(fake_label[i]) #主要是这一步 将之前列表数据转为str才能加入列表
        line_new = line_new + ' ' + b +'\n'
        i += 1
       # print(line_new)
        ff.write(line_new) #写入一个新文件中

def add_fake_test_labels(fake_label,args):
    ss = args.dset.split('2')[0]
    tt = args.dset.split('2')[1]
    if ss == 'a':
         s = 'Art'
    ...
    fake_label_path = 'C:/Users/10189/Desktop/CoDA-main1/data_press/fake_label_data/test/{}_addfake_test.txt'.format(t)
    test_label_path = 'C:/Users/10189/Desktop/CoDA-main1/data_press/{}_test.txt'.format(t)
    ff = open(fake_label_path,'w')  #打开一个文件，可写模式
    with open(test_label_path,'r') as f:  #打开一个文件只读模式
         line = f.readlines()
    i = 0
...
def split_train_test(class_num, datapath_list, train_ratio=0.7, valid_ratio=0.1):
    data_list_by_category = [[] for _ in range(class_num)]
    for i in range(len(datapath_list)):
        line = datapath_list[i].strip()
        ...
        l = str(line.split(' ')[0])
        #print(l)
        data_list_by_category[label].append(l +' '+ z  + '\n')
    train, valid, test = [], [], []
    for i in range(len(data_list_by_category)):
        data_list_by_category_i = data_list_by_category[i]
        random.shuffle(data_list_by_category_i)
