...
class ImageList_idx(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img1, target, self.domain_id, index,path
...
def office_load_idx(args):
    ...
    if args.dataset == 'office_home':
        ...
        if os.path.exists(s_tr_path) and os.path.exists(s_ts_path):
            ...
            s_tr, s_ts = './data_split/office31/{}.txt'.format(
                s), './data_split/office31/{}.txt'.format(s)
            txt_src = open(s_tr).readlines()
            s_tr, s_ts = split_train_test_by_category(args.class_num,
                                                    txt_src,
                                                    train_ratio=0.8)
            with open('./data_split/office31/'+s+'_train.txt', 'w') as f:
                for i in s_tr:
                    f.write(i)
            with open('./data_split/office31/'+s+'_test.txt', 'w') as f:
                for i in s_ts:
                    f.write(i)
        if os.path.exists(t_tr_path) and os.path.exists(t_ts_path):
            t_tr, t_ts = open(t_tr_path).readlines(), open(t_ts_path).readlines()
        else:
            t_tr, t_ts = './data_split/office31/{}.txt'.format(
                t), './data/office31/{}.txt'.format(t)
            txt_tgt = open(t_tr).readlines()
            
            t_tr, t_ts = split_train_test_by_category(args.class_num,
                                                    txt_tgt,
                                                    train_ratio=0.8)
            with open('./data_split/office31/'+t+'_train.txt', 'w') as f:
                for i in t_tr:
                    f.write(i)
            with open('./data_split/office31/'+t+'_test.txt', 'w') as f:
                for i in t_ts:
                    f.write(i)
        ...
        if os.path.exists(s_tr_path) and os.path.exists(s_ts_path):
            ...
            s_tr, s_ts = './data_split/image_CLEF/{}.txt'.format(
                s), './data_split/image_CLEF/{}.txt'.format(s)
            txt_src = open(s_tr).readlines()
            s_tr, s_ts = split_train_test_by_category(args.class_num,
                                                    txt_src,
                                                    train_ratio=0.8)
            with open('./data_split/image_CLEF/'+s+'_train.txt', 'w') as f:
                for i in s_tr:
                    f.write(i)
            with open('./data_split/image_CLEF/'+s+'_test.txt', 'w') as f:
                for i in s_ts:
                    f.write(i)
        if os.path.exists(t_tr_path) and os.path.exists(t_ts_path):
            t_tr, t_ts = open(t_tr_path).readlines(), open(t_ts_path).readlines()
        else:
            t_tr, t_ts = './data_split/image_CLEF/{}.txt'.format(
                t), './data/image_CLEF/{}.txt'.format(t)
            txt_tgt = open(t_tr).readlines()
            
            t_tr, t_ts = split_train_test_by_category(args.class_num,
                                                    txt_tgt,
                                                    train_ratio=0.8)
            with open('./data_split/image_CLEF/'+t+'_train.txt', 'w') as f:
                for i in t_tr:
                    f.write(i)
            with open('./data_split/image_CLEF/'+t+'_test.txt', 'w') as f:
                for i in t_ts:
                    f.write(i)
        ...
        s_tr_path = './data_split/Adaptiope/'+s+'_train.txt'
        s_ts_path = './data_split/Adaptiope/' + s + '_test.txt'
        t_tr_path = './data_split/Adaptiope/' + t + '_train.txt'
        t_ts_path = './data_split/Adaptiope/' + t + '_test.txt'

        if os.path.exists(s_tr_path) and os.path.exists(s_ts_path):
            s_tr, s_ts = open(s_tr_path).readlines(), open(s_ts_path).readlines()
        else:
            s_tr, s_ts = './data_split/Adaptiope/{}.txt'.format(
                s), './data_split/Adaptiope/{}.txt'.format(s)
            txt_src = open(s_tr).readlines()
            s_tr, s_ts = split_train_test_by_category(args.class_num,
                                                    txt_src,
                                                    train_ratio=0.8)
            with open('./data_split/Adaptiope/'+s+'_train.txt', 'w') as f:
                for i in s_tr:
                    f.write(i)
            with open('./data_split/Adaptiope/'+s+'_test.txt', 'w') as f:
                for i in s_ts:
                    f.write(i)
        if os.path.exists(t_tr_path) and os.path.exists(t_ts_path):
            t_tr, t_ts = open(t_tr_path).readlines(), open(t_ts_path).readlines()
        else:
            t_tr, t_ts = './data_split/Adaptiope/{}.txt'.format(
                t), './data/Adaptiope/{}.txt'.format(t)
            txt_tgt = open(t_tr).readlines()
            
            t_tr, t_ts = split_train_test_by_category(args.class_num,
                                                    txt_tgt,
                                                    train_ratio=0.8)
            with open('./data_split/Adaptiope/'+t+'_train.txt', 'w') as f:
                for i in t_tr:
                    f.write(i)
            with open('./data_split/Adaptiope/'+t+'_test.txt', 'w') as f:
                for i in t_ts:
                    f.write(i)
        ...
        s_tr_path = './data_split/PACS/'+s+'_train.txt'
        s_ts_path = './data_split/PACS/' + s + '_test.txt'
        t_tr_path = './data_split/PACS/' + t + '_train.txt'
        t_ts_path = './data_split/PACS/' + t + '_test.txt'

        if os.path.exists(s_tr_path) and os.path.exists(s_ts_path):
            s_tr, s_ts = open(s_tr_path).readlines(), open(s_ts_path).readlines()
        else:
            s_tr, s_ts = './data_split/PACS/{}.txt'.format(
                s), './data_split/PACS/{}.txt'.format(s)
            txt_src = open(s_tr).readlines()
            s_tr, s_ts = split_train_test_by_category(args.class_num,
                                                    txt_src,
                                                    train_ratio=0.8)
            with open('./data_split/PACS/'+s+'_train.txt', 'w') as f:
                for i in s_tr:
                    f.write(i)
            with open('./data_split/PACS/'+s+'_test.txt', 'w') as f:
                for i in s_ts:
                    f.write(i)
        if os.path.exists(t_tr_path) and os.path.exists(t_ts_path):
            t_tr, t_ts = open(t_tr_path).readlines(), open(t_ts_path).readlines()
        else:
            t_tr, t_ts = './data_split/PACS/{}.txt'.format(
                t), './data/PACS/{}.txt'.format(t)
            txt_tgt = open(t_tr).readlines()
            
            t_tr, t_ts = split_train_test_by_category(args.class_num,
                                                    txt_tgt,
                                                    train_ratio=0.8)
            with open('./data_split/PACS/'+t+'_train.txt', 'w') as f:
                for i in t_tr:
                    f.write(i)
            with open('./data_split/PACS/'+t+'_test.txt', 'w') as f:
                for i in t_ts:
                    f.write(i)
    #print(s_tr)
    prep_dict = {}
    ...
    dset_loaders["source_train"] = DataLoader(train_source,
    ...
    dset_loaders["target_train"] = DataLoader(train_target,
                                        batch_size=train_bs * 3,
                                        # collate_fn=collate_fn,
                                        shuffle=False,
                                        num_workers=args.worker,
